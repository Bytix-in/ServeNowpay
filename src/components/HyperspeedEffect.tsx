...
class App {
  ...
  init() {
    ...
    this.container.addEventListener("touchcancel", this.onMouseUp);

    this.tick();
  }

  onMouseDown(ev: MouseEvent | TouchEvent) {
    if (this.options.onSpeedUp) this.options.onSpeedUp(ev as MouseEvent);
    this.fovTarget = this.options.fovSpeedUp;
    this.speedUpTarget = this.options.speedUp;
  }

  onMouseUp(ev: MouseEvent | TouchEvent) {
    if (this.options.onSlowDown) this.options.onSlowDown(ev as MouseEvent);
...
const HyperspeedEffect: FC<HyperspeedProps> = ({ effectOptions = {}, className = '' }) => {
